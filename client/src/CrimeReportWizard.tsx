...
export default function CrimeReportWizard({ service, onBack }: { service: Service; onBack: () => void }) {
  ...
  const setDetail=(k:string,v:unknown)=>setDraft(current=>({...current,details:{...current.details,[k]:v}}))
  const d=draft.details as Record<string,any>

  const canContinue=useMemo(()=>{
    if(step===0)return d.emergencyConfirmed===true && d.suspectGone===true && d.noKnownSuspectException===true
  ...
  async function checkAddress(){
    ...
    try{
      ...
      if(!r.valid){setDetail('jurisdictionConfirmed',false);setError(r.warning||'Address could not be validated for Charlotte/Mecklenburg.');return}
      setDraft(current=>({...current,location:r.normalizedAddress||current.location,details:{...current.details,jurisdictionConfirmed:r.inCharlotteMecklenburg,latitude:r.latitude,longitude:r.longitude,addressWarning:r.warning||''}}))
  ...
  return <main className="wrap wizard">
    ...
    <section className="panel">
      ...
      {step===2&&<><span className="eyebrow">STEP 3</span><h2>Reporter information</h2><Two a={<Field label="Full name *"><input value={draft.contact.name??''} onChange={e=>setDraft(current=>({...current,contact:{...current.contact,name:e.target.value}}))}/></Field>} b={<Field label="Email *"><input type="email" value={draft.contact.email??''} onChange={e=>setDraft(current=>({...current,contact:{...current.contact,email:e.target.value}}))}/></Field>}/><Two a={<Field label="Phone"><input value={draft.contact.phone??''} onChange={e=>setDraft(current=>({...current,contact:{...current.contact,phone:e.target.value}}))}/></Field>} b={<Field label="Preferred contact"><select value={draft.contact.preferredMethod??''} onChange={e=>setDraft(current=>({...current,contact:{...current.contact,preferredMethod:e.target.value}}))}><option value="">No preference</option><option>Email</option><option>Phone</option><option>Text</option></select></Field>}/>}</>}
      {step===3&&<><span className="eyebrow">STEP 4</span><h2>Incident location and time</h2><Field label="Incident address *"><div className="inline"><input value={draft.location} onChange={e=>setDraft(current=>({...current,location:e.target.value,details:{...current.details,jurisdictionConfirmed:false}}))} placeholder="Street address, Charlotte NC"/><button type="button" className="btn secondary" onClick={checkAddress} disabled={busy}>{busy?'Checking…':'Validate address'}</button></div></Field>{d.jurisdictionConfirmed===true&&<div className="valid">✓ Address matched in Charlotte/Mecklenburg</div>}<Two a={<Field label="When did it happen? *"><input type="datetime-local" value={d.occurredAt??''} onChange={e=>setDetail('occurredAt',e.target.value)}/></Field>} b={<Field label="When did you discover it?"><input type="datetime-local" value={d.discoveredAt??''} onChange={e=>setDetail('discoveredAt',e.target.value)}/></Field>}/><Field label="Location details"><input value={d.locationDetails??''} onChange={e=>setDetail('locationDetails',e.target.value)} placeholder="Apartment/unit, parking level, business name, landmark"/></Field></>}
