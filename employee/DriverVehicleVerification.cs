...
public static class DriverVehicleVerification
{
    public static void Map(WebApplication app)
    {
        ...
        app.MapPost("/identity-verification/license", async (HttpContext ctx, IAntiforgery anti, IHttpClientFactory factory) =>
        {
            ...
            return Html(Page("Verification result",$"""<main class='wrap'><a class='back' href='/identity-verification'>← New verification</a><section class='page-head'><div><span class='eyebrow'>IDENTITY COMPARISON</span><h1>{(overall?"Match":"Mismatch")}</h1></div></section><section class='card'><dl><dt>Registered owner</dt><dd>{H(session.OwnerName)} · {H(session.OwnerAddress)}</dd><dt>Presented driver</dt><dd>{H(presentedName)} · {H(address)}</dd><dt>DMV driver record</dt><dd>{H(verifiedName)} · {H(verifiedAddress)}</dd></dl>{next}</section></main>"""));
        }).RequireAuthorization();
